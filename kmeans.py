...
def get_closest_centroid(point, centroids_dict):
    ...
    centroid_list = list(centroids_dict.keys())[0]
    closest_centroid = euclidean_distance(point, centroids_dict[centroid_list])
    closest = ''
    for x in centroids_dict:
        d = euclidean_distance(point, centroids_dict[x])
        if closest_centroid >= d:
            closest_centroid = d
            closest = x
    return closest
...
def update_assignment(list_of_points, centroids_dict):
    ...
    for x in list_of_points:
        nearest_centroid = get_closest_centroid(x, centroids_dict)
        if nearest_centroid not in new_dictionary:
            new_dictionary[nearest_centroid] = [x]
        else:
            new_dictionary[nearest_centroid].append(x)
...
def mean_of_points(list_of_points):
    ...
    for x in range(len(list_of_points[0])):
        ...
        for y in range(len(list_of_points)):
            ...
            average = sum / len(list_of_points)
        float_list.append(average)
    return float_list
...
def update_centroids(assignment_dict):
    ...
    """
    new_dict = {}
    for (x, y) in assignment_dict.items():
        new_dict[x] = mean_of_points(y)
    return new_dict
