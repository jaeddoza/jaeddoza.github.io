...
def read_grid(file_path):
    ...
    grid_counter = []
    new_type_file = open(file_path)
    for line_of_text in new_type_file:
        grid_counter.append(csv_line_to_list(line_of_text))
    new_type_file.close()
    return grid_counter
...
def get_pixel_at(pixel_grid, i, j):
    ...
    """
    if i > -1 and j > -1:
        if i < len(pixel_grid) and j < len(pixel_grid[0]):
...
def average_of_surrounding(pixel_grid, i, j):
    ...
    for r in num_rows:
        for c in num_cols:
            pixel_sum += get_pixel_at(pixel_grid, r, c)
...
def blur(pixel_grid):
    ...
    for i in range(0, len(pixel_grid)):
        ...
        for j in range(len(pixel_grid[0])):
            new_list.append(average_of_surrounding(pixel_grid, i, j))
        blurred_grid.append(new_list)
...
# the input file.
out_image_filename = input_filename + '_blurry.png'
out_grid_filename = input_filename + '_blurry_grid.txt'

# Step D: Apply the blur algorithm

geeked = blur(input_grid)

# Step E: Write the result to both output files

write_image(out_image_filename, geeked)
write_grid(out_grid_filename, geeked)
